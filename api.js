...
(function(){
  ...
  function findEpisodes(value,out=[]){
    const seen=new Set();
    function walk(v,season='1'){
      if(!v) return;
      if(Array.isArray(v)){v.forEach(x=>walk(x,season));return;}
      if(typeof v!=='object') return;
      ...
      const title=first(v,['title','name','episode_title'],`Episodio ${num??out.length+1}`);
      const s=first(v,['season','season_number','seasonNumber'],season);
      if(url && (num!==null || /episode|episodio/i.test(String(title)))){
        const key=JSON.stringify([s,num,title,url]);
        if(!seen.has(key)){seen.add(key);out.push({season:String(s),number:String(num??out.length+1),title:String(title),url:String(url),raw:v});}
      }
      Object.entries(v).forEach(([k,x])=>walk(x,/season/i.test(k)&&typeof x!=='object'?String(x):season));
    }
    walk(value);
    return out.sort((a,b)=>Number(a.season)-Number(b.season)||Number(a.number)-Number(b.number));
  }
  ...
  window.NVApi={getJSON,list,detail,resolve,resolveEpisode,genres,normalize,findUrls,findQualityProxyUrls,findEpisodes,providers:A.providers};
})();
